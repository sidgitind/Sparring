...
4. Summary regenerates on the next ticket-open event after new messages
   ...
   not immediately)
5. Median time-to-first-action (time from ticket open to the engineer's
   first reply or status change) decreases by at least 40% within
   30 days of launch, measured against a comparable
   pre-launch baseline (same ticket volume/complexity mix)
6. If, within any rolling 30 days window post-launch, either
   (a) the thumbs-down rate on generated summaries exceeds 10%, or
   (b) sampled summary accuracy falls below the 90% threshold in AC2,
   the feature is flagged for review within 5 business days
   and may be disabled pending investigation
7. If summary generation fails, each subsequent ticket-open event
   automatically retries generation until it either succeeds or 3 total
   consecutive attempts have failed; after the 3rd consecutive failure,
   no further automatic retry occurs until a new message arrives on the
   thread
8. The summary panel's loading state resolves to the existing failure
   display if no response is received within 20 seconds of ticket open
9. If a thumbs up/down vote fails to save, the selection reverts to its
   prior state and a brief, non-blocking message indicates it wasn't
   saved — no blocking dialog or separate error page
