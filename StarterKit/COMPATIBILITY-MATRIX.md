...
> Which personas work together, which create productive tension,
> and how to resolve conflicts when they disagree.

---

## The Standard Pipeline

The default sequence for most features:

```
[PM: Amazon or Google] → [Architect] → [UI Designer] → [QA Functional] → [QA NFQ]
```

This order is not arbitrary. Each persona's output is the next persona's input.
...
Running them out of order degrades output quality. Specific consequences below.
...
A Google PM may add: "p95 login time < 200ms."
QA NFQ will flag: "Google OAuth p95 latency is 300–500ms. This SLO
is not achievable for the login flow as specified."
...
the architecture (caching, pre-authentication, etc.) to make it
achievable. Do not leave an impossible SLO in the spec — the agent
cannot build to a target that physics prevents.
...
- QA Functional auth-only security: standard OAuth, not enterprise SSO

### Combination B: SaaS B2B (the Flowdesk model)
```
...
- OAuth security checklist items (these are OWASP requirements)
